...
def vid2jpg_train_mp(train_file):
    class_name, video_name, start, end, split, is_cc = train_file.split(',')
    ...
    # videofile_name = '{vname}_{0:06d}_{0:06d}.mp4'.format(vname=video_name, start, end)
    file_name = os.path.join(train_video_path, videofile_name)
    ...
    try:
        ...
        return
    # cmd = 'ffmpeg -i \"{}\" -vf \"minterpolate=fps=30:mi_mode=mci:mc_mode=aobmc:me_mode=bidir:vsbmc=1\"  -threads 40 -qscale:v 2 \"{}/img_%05d.jpg\"'.format(
    #     video_file_path,
    #     dst_directory_path)
...
def vid2jpg_val_mp(val_file):
    ...
    try:
        ...
        print(dst_directory_path)
        return
    # cmd = 'ffmpeg -i \"{}\" -vf \"minterpolate=fps=30:mi_mode=mci:mc_mode=aobmc:me_mode=bidir:vsbmc=1\"  -threads 40 -qscale:v 2 \"{}/img_%05d.jpg\"'.format(video_file_path,
    #                                                                                          dst_directory_path)
    # subprocess.call(cmd, shell=True, stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
with val_list as vfile:
    for line in vfile:
        # p.map(vid2jpg_val_mp, line) #storing everything in memory!
        vid2jpg_val_mp(line)
