...
def load_data(num_class, input_dir):
    ...
    train_dataset = BasicDataset(train_list, input_dir, args.train_num_frames,\
                 cls_num=num_class, train_mode=True, multilabel=True, lb=args.lb)
    ...
    return train_dataloader, val_dataloader, sampling_prob, smooth_factor
...
def main():
    ...
    train_loader, val_loader, sampling_prob, smooth_factor = load_data(num_class, input_dir)
    ...
    MHA = utils.find_class_by_name('MultiHeadAttention_Dynamic_Aggregator', [models])(n_head=4, d_model=2048, d_k=512, d_v=512, dropout=0.1).cuda()
