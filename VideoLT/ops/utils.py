...
class LTAverageMeter(object):
    def __init__(self, indices):
        ...
        self.val = torch.zeros(1004).float()
        self.avg = torch.zeros(1004).float()
        self.sum = torch.zeros(1004).float()
        self.count = torch.zeros(1004).float()
    ...
    def update(self, val, target, n=1):
        for idx, ml in enumerate(target):
            if len(ml) == 1:
                ...
                for iidx, l in enumerate(ml):
                    self.val[int(l)] = val[idx]
                    self.sum[int(l)] += val[idx]
                    self.count[int(l)] += 1
        self.avg = self.sum / self.count
...
class LTconfMeter(object):
    def __init__(self, indices):
        ...
        self.cval = torch.zeros(1004).float()
        self.cavg = torch.zeros(1004).float()
        self.csum = torch.zeros(1004).float()
        self.ccount = torch.zeros(1004).float()
    ...
    def update(self, val, target, n=1):
        ...
        for idx, ml in enumerate(target):
            if len(ml) == 1:
                ...
                # continue
                for iidx, l in enumerate(ml):
                    self.cval[int(l)] = val[idx][int(l)]
                    self.csum[int(l)] += val[idx][int(l)]
                    self.ccount[int(l)] += 1
